...
export const STATUS_SEARCH_DOWLOAD_PENDING = 'PENDING';
export const STATUS_SEARCH_DOWLOAD_ANSWER_IS_NULL = 'NULL';
export const STATUS_SEARCH_DOWLOAD_ERROR = 'ERROR';
export const STATUS_SEARCH_DOWLOAD_FINISH = 'FINISH';
export const STATUS_SEARCH_DOWLOAD_WAIT = 'WAIT';
...
export const searchSlice = createSlice({
  name: 'search',
  initialState: {
    statusDownloadWork: STATUS_SEARCH_DOWLOAD_WAIT,
    seachList: {},
    pageList: {},
  },
  reducers: {
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchSearchListWork.pending, (state) => {
      state.statusDownloadWork = STATUS_SEARCH_DOWLOAD_PENDING;
    });
    builder.addCase(fetchSearchListWork.rejected, (state, action) => {
      state.seachList[action.meta.arg.url] = null;
      state.statusDownloadWork = STATUS_SEARCH_DOWLOAD_ERROR;
    });
    builder.addCase(fetchSearchListWork.fulfilled, (state, action) => {
      if (action.payload.arrayWorks.length === 0) {
        state.statusDownloadWork = STATUS_SEARCH_DOWLOAD_ANSWER_IS_NULL;
      } else {
        state.statusDownloadWork = STATUS_SEARCH_DOWLOAD_FINISH;
      }
    });
  },
});
